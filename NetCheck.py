...
#  NetCheck
#
#  - Monitor the network connection via pings
#  - written for Python v3.5+
#
# 11Mar2017 A. Cooper v0.1
#  - initial version
#
#------------------------------------------------------------------------------
verStr= "NetCheck v0.1"

#-- constants -----------------------------------------------------------------
pingTarg= "192.168.1.1"
pingSlow= 100
pingBad=  300
...
class Application(Frame):
  ...
  def createWidgets(self):
    # action buttons
    self.startButton=       Button(self,text="Start",width=10,bg=colStart,command=self.cmdStart,font=("Helvetica","12"))
    ...
    self.lookOptions=       ("Sample","30min","1hour","2hours","4hours","12hours","1day","2days","10days","30days")
  ...
  def drawGraph(self):
    ...
    if self.lookSelect.get()=='Sample': #plot one pixel per sample on chart
      ...
      self.graph.create_text(self.graphWidth,zero+1,text="{:%Y-%m-%d %H:%M:%S}".format(data[len(data)-1][0]),anchor=NE)
      ...
      for point in data:
        ...
        if point[1]:
          ...
          dLine= self.graph.create_line(x,zero,x,0,fill=colFail)
      self.graph.create_text(xMarg,zero+1,text="{:%Y-%m-%d %H:%M:%S}".format(datetime.fromtimestamp(start)),anchor=NW)
      self.graph.create_text(self.graphWidth,zero+1,text="{:%Y-%m-%d %H:%M:%S}".format(datetime.fromtimestamp(stop)),anchor=NE)
    #draw axis
